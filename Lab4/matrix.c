...
double *sendData2(matrix A, MPI_Comm world) {
	int worldSize, rank;
	...
	*/
	...
	double *arr;
	arr = malloc(A.cols*sendcounts[rank]*sizeof(double));

	MPI_Datatype rows;	
	MPI_Type_contiguous(A.cols, MPI_DOUBLE, &rows);
...
matrix multiplicationType(matrix A, matrix B, MPI_Comm world) {
	int worldSize, rank;
...
matrix powerMethod(matrix A, int amount, const char *fname, MPI_Comm world) {
	int worldSize, rank;
	...
	matrix LA;
	LA.data = a;
	LA.rows = sendcounts[rank] / A.cols;
	LA.cols = A.cols; 
	
	matrix x;
	initMatrix(&x, A.rows, 1);
	for (int i = 0; i < A.rows; i++) {
		ACCESS(x, i, 0) = 1;
	}
	...
	matrix LX;
	initMatrix(&LX, LA.rows, 1);

	int *newSend; 
	newSend = malloc(worldSize*sizeof(int));
	
	for (int i = 0; i < worldSize; i++) {
		newSend[i] = sendcounts[i] / A.cols * x.cols; 
	}
	...
	while (count < amount) {
		for (int i = 0; i < LA.rows; i++) {
			for (int j = 0; j < x.cols; j++) {
				sum = 0;
				for (int k = 0; k < LA.cols; k++) {
					sum += ACCESS(LA, i, k) * ACCESS(x, k, j);
				}
				//arr2[count] = sum;
				ACCESS(LX, i, j) =  sum;
				//count++;
		...
		*/
		...
		MPI_Allgatherv(
			LX.data, // sendbuf
			newSend[rank], // sendcount
			rows,   // sendtype
			x.data, // recvbuff
			newSend, //recvcounts (arr)
			newDispl,
			MPI_DOUBLE,    // displ
			world         // recvtype
							// comm
		...
		*/
		MPI_Allgatherv(LX.data, newSend[rank], MPI_DOUBLE, x.data, newSend, newDispl, MPI_DOUBLE, world);
		for (int z = 0; z < x.rows; z++) {
			norm += ACCESS(x, z, 0) * ACCESS(x, z, 0);
		}
		norm = sqrt(norm);		
		if (rank == 0) printf("norm %0.6f\n", norm);
		for (int z = 0; z < x.rows; z++) {
			ACCESS(x, z, 0) /= norm;
		}
		if (rank == 0) printMatrix(x);
